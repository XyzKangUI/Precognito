...
function AM_Core.Enable()
    ...
    if (not AM_Core.Silent) then
        if Precognito.db.profile.syncMsg then
            AM_Core.SetVerbose();
        else
            AM_Core.SetSilent();
        end
    ...
    if Precognito.db.profile.syncMsg then
        ...
        AM_Core:RegisterComm("Absorbs_US", AM_Events.OnUnitStatsReceived);
        AM_Core:RegisterComm("Absorbs_SC", AM_Events.OnScalingReceived);
    end
...
function AM_Core.SendUnitStats()
    if (curChatChannel) then
        ...
        if ((curAP ~= lastAP) or (curSP ~= lastSP)) then
            AM_Core:SendCommMessage("Absorbs_US", AM_Core:Serialize(playerGUID, playerClass, curAP, curSP), curChatChannel);
...
function AM_Core.SendScaling()
    if (curChatChannel) then
        AM_Core:SendCommMessage("Absorbs_SC", AM_Core:Serialize(playerGUID, playerClass, AM_Events.OnScalingEncode()), curChatChannel);
...
function AM_Core.SetVerbose()
    AM_Core.RegisterEvent("GROUP_ROSTER_UPDATE");
...
function AM_Core.SetSilent()
    AM_Core.UnregisterEvent("GROUP_ROSTER_UPDATE");
...
function AM_Events.STATS_CHANGED()
    ...
    if (curChatChannel) and Precognito.db.profile.syncMsg then
...
function AM_Events.OnUnitStatsReceived(prefix, text, distribution, target)
    ...
    if (not (success and guid and class and ap and sp)) then
        return ;
    end
...
function AM_Events.OnScalingReceived(prefix, text, distribution, target)
    ...
    if (not (success and guid and class and inScaling)) then
        return ;
    end
...
AM_Events.GROUP_ROSTER_UPDATE = AM_Events.GROUPING_CHANGED;
...
function AM_Public.PrintProfiling()
    ...
    AM_Core.Print("Mem: " .. string.format("%.3f", GetAddOnMemoryUsage("AbsorbsMonitor")) .. " kB");
    AM_Core.Print("Time: " .. string.format("%.3f", GetAddOnCPUUsage("AbsorbsMonitor")) .. " ms");
    ...
    for k, v in pairs(funcTable) do
        ...
        if (v_type == "function") then
            ...
            AM_Core.Print(k .. " (#" .. count .. "): " .. string.format("%.4f", time_self) .. " / " .. string.format("%.4f", time_combined));
        end
    end
end
...
local function priest_ApplyScaling(guid, level, baseFactor, spFactor, daFactor)
    ...
    for k, v in pairs(priest_PWS_Ranks) do
        if (v[2] <= level) then
            ...
            if (v[2] < (level - 5)) then
                if (level == 80) then
                    rankSP = math.max(spFactor * ((v[2] * 0.045228921) - 2.381389768), 0);
                else
                    rankSP = 0;
                end
            else
...
local function items_Valanyr_Create(sourceGUID, sourceName, destGUID, destName, spellId, destEffects)
    ...
    return math.min(20000, existing + charge), 1.0;
end
...
AM_Core.Effects = {
    ...
    -- Essence of Gossamer
    [60218] = { 5.0, 10, function()
        return 4000, 1.0;
    end, items_EssenceOfGossamer_Hit },
    -- Corroded Skeleton Key
    [71586] = { 1.0, 10, function()
        return 6400, 1.0;
    end, generic_Hit },
    -- Phaseshift Bulwark
    [36481] = { 1.0, 4, function()
        return 100000, 1.0;
    end, generic_Hit },
    -- Darkmoon Card: Illusion
    [57350] = { 1.0, 6, function()
        return 1500, 1.0;
    end, generic_Hit },
    -- Mark of the Dragon Lord
    [17252] = { 1.0, 1800, function()
        return 500, 1.0;
    end, generic_Hit },
    ...
    -- Argussian Compass
    [39228] = { 5.0, 20, function()
        return 1150, 1.0;
    end, items_ArgussianCompass_Hit },
    -- Runed Fungalcap
    [31771] = { 1.0, 20, function()
        return 440, 1.0;
    end, generic_Hit },
    -- Truesilver Champion
    [9800] = { 1.0, 60, function()
        return 175, 1.0;
    end, generic_Hit },
    -- Gnomish Harm Prevention Belt
    [13234] = { 1.0, 600, function()
        return 500, 1.0;
    end, generic_Hit },
    -- Nigh Invulnerability Belt
    [30458] = { 1.0, 8, function()
        return 4000, 1.0;
    end, generic_Hit },
    ...
    [29674] = { 1.0, nil, function()
        return 1000, 1.0;
    end, generic_Hit },
    -- Greater Ward of Shielding
    [29719] = { 1.0, nil, function()
        return 4000, 1.0;
    end, generic_Hit },
